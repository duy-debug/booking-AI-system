...
from app.domain.booking import CourseType, TherapistPreference, TherapistPreferenceType
...
) -> tuple[httpx.AsyncClient, HTTPBookingGateway]:
...
def test_rejects_empty_base_url_and_invalid_timeout() -> None:
    ...
    with pytest.raises(ValueError, match="base URL"):
        HTTPBookingGateway(client=client, base_url="  ")
    with pytest.raises(ValueError, match="timeout"):
        HTTPBookingGateway(client=client, base_url="https://pos.example", timeout_seconds=0)
...
async def test_search_services_sends_only_pos_filters_and_maps_course_types(
    ...
    assert services[0].course_type is CourseType.MAIN
...
async def test_create_booking_sends_exact_payload_and_maps_child_reservations() -> None:
    ...
    assert result.booking.service.service_id == MAIN_COURSE_ID
